...
const RequestedDesigners = () => {
  const { requestedDesigners, isEmpty, isLoading } = useLiveCollection("requestedDesigners")
  ...
  const markAsDone = async (request) => {
    ...
    await firebase.db.collection("requestedDesigners").doc(request.id).delete()
  }
    ...
    <OuterContainer>
      ...
      {isEmpty ? (
        ...
        requestedDesigners.map((request) => (
          <DesignerRequest key={request.id}>
            <h3>{request.name}</h3>
