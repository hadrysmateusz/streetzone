...
const EditPost = ({ match }) => {
  ...
  const onSubmit = async ({ section, mainImage, title, author, mainContent, dropsAt }, actions) => {
    try {
      ...
      if (!mainImage.storageRef) {
        ...
        const mainImageSnap = await firebase.uploadFile(`blog-photos/${imageId}`, mainImage.data)
            ...
            return (
              <form onSubmit={handleSubmit}>
                {values.section && values.section !== "Dropy" && (
                  <Field name="author">
                    {({ input, meta }) => {
                      ...
                      return <Input {...input} type="text" placeholder="Author" error={error} />
                    }}
                  </Field>
                )}
                ...
                <Field name="mainContent">
                  {({ input, meta }) => {
                    const error = meta.error && meta.touched ? meta.error : null
                    return <Textarea {...input} placeholder="Main text content" error={error} />
                  }}
                </Field>
                  ...
                  <>
                    ...
                    <Field name="dropsAt">
                      {({ input, meta }) => {
                        const error = meta.error && meta.touched ? meta.error : null
                        return (
                          <Datetime {...input} error={error} input={false} timeFormat="HH:mm" />
                        )
                      }}
                    </Field>
                  </>
                ...
                <Field name="tags" type="select">
                  {({ input, meta }) => {
                    const error = meta.error && meta.touched ? meta.error : null
                    return <MultiTextInputFinalform {...input} placeholder="Tagi" error={error} />
                  }}
                </Field>
