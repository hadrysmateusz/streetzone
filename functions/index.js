...
const ALGOLIA_ADMIN_KEY = functions.config().algolia.app_id
...
exports.onItemCreated = functions.firestore.document(`items/{id}`).onCreate((snap, context) => {
  algoliaAdd(snap, context, ITEMS_MARKETPLACE_DEFAULT_ALGOLIA_INDEX)
})

exports.onItemUpdated = functions.firestore.document(`items/{id}`).onUpdate((snap, context) => {
  algoliaUpdate(snap, context, ITEMS_MARKETPLACE_DEFAULT_ALGOLIA_INDEX)
})

exports.onItemDeleted = functions.firestore.document(`items/{id}`).onDelete((snap, context) => {
...
exports.onBlogPostCreated = functions.firestore.document(`posts/{id}`).onCreate((snap, context) => {
  algoliaAdd(snap, context, BLOG_POST_ALGOLIA_INDEX)
})

exports.onBlogPostUpdated = functions.firestore.document(`posts/{id}`).onUpdate((snap, context) => {
  algoliaUpdate(snap, context, BLOG_POST_ALGOLIA_INDEX)
})

exports.onBlogPostDeleted = functions.firestore.document(`posts/{id}`).onDelete((snap, context) => {
...
exports.onDealCreated = functions.firestore.document(`deals/{id}`).onCreate((snap, context) => {
  algoliaAdd(snap, context, DEALS_ALGOLIA_INDEX)
})

exports.onDealUpdated = functions.firestore.document(`deals/{id}`).onUpdate((snap, context) => {
  algoliaUpdate(snap, context, DEALS_ALGOLIA_INDEX)
})

exports.onDealDeleted = functions.firestore.document(`deals/{id}`).onDelete((snap, context) => {
...
exports.onDropCreated = functions.firestore.document(`drops/{id}`).onCreate((snap, context) => {
  algoliaAdd(snap, context, BLOG_DROP_ALGOLIA_INDEX)
})

exports.onDropUpdated = functions.firestore.document(`drops/{id}`).onUpdate((snap, context) => {
  algoliaUpdate(snap, context, BLOG_DROP_ALGOLIA_INDEX)
})

exports.onDropDeleted = functions.firestore.document(`drops/{id}`).onDelete((snap, context) => {
...
exports.removeItemImages = functions.firestore.document(`items/{id}`).onDelete(async (snap) => {
  ...
  const removeFile = async (name) => {
    try {
      ...
      console.error(`There was a problem with deleting ${name} it might not have been deleted`, err)
    }
  }
...
exports.removeItemFromUser = functions.firestore
  ...
  .onDelete(async (snap, context) => {
    ...
    return db.collection("users").doc(userId).update({ items: newItems })
  })
...
exports.removeUserItems = functions.firestore
  ...
  .onDelete((userSnap, context) => {
    ...
    const promises = userData.items.map((item) => db.collection("items").doc(item).delete())
...
const writeUrlsToDb = async (userId, urls) => {
  ...
  return db.collection("users").doc(userId).update({ profilePictureURLs })
}
...
class PromotingLevel {
  ...
  formatForDb(oldData) {
    ...
    const promotedUntil = moment(startAt).add(this.duration, "days").valueOf()
...
class PromotingManager {
  ...
  async promoteItem(itemId, level) {
    ...
    const oldItemSnap = await db.collection("items").doc(itemId).get()
    ...
    return db.collection("items").doc(itemId).update(formattedData)
  }
}
...
exports.dropNotification = functions.pubsub
  ...
  .onRun(async (context) => {
    ...
    const nowTimestamp = moment().subtract(12, "hours").valueOf()
...
exports.confirmDesignerAdded = functions.https.onCall(async (data, context) => {
  ...
  // Get the list of device notification tokens.
  const tokensSnap = await db.doc(`users/${userId}`).collection("notificationTokens").get()
